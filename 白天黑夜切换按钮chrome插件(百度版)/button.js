	  ...
	  let specificEm = document.querySelectorAll('.group-sub-abs_N-I8P em');
	  let pageButtonBg = document.querySelectorAll('span.page-item_M4MDr.pc');
	  let pageInputBg = document.querySelectorAll('span.bg.s_ipt_wr.new-pmd.quickdelete-wrap');
	  let inputColor = document.querySelectorAll('input');

	  let allDivs = document.querySelectorAll('div');
	  let specificDivs = Array.from(allDivs).filter(div => {
		return !div.classList.contains('img-mask_2AwMa') &&
			   !div.classList.contains('rs-normal-width_2T91A') &&
			   !div.classList.contains('c-line-clamp1') &&
			   !div.classList.contains('c-color-link') &&
			   !div.classList.contains('imgbox');
	  });
	  let bottomDiv = Array.from(allDivs).filter(div => {
		const bottomDiv1 = div.classList.contains('page-inner_2jZi2');
		const bottomDiv2 = div.classList.contains('foot-container_2X1Nt');
		return bottomDiv1 || bottomDiv2;
	  });

	  let allA = document.querySelectorAll('a');
	  let specificA = Array.from(allA).filter(a => {
		const a1 = a.classList.contains('c-gap-top-xsmall') && a.classList.contains('item_3WKCf');
		const a2 = a.classList.contains('rs-link_2DE3Q') && a.classList.contains('c-line-clamp1') && a.classList.contains('c-color-link');
		return a1 || a2;
	  });
	  let rightA = Array.from(allA).filter(a => {
		const rightA1 = a.classList.contains('toindex');
		const rightA2 = a.classList.contains('pf');
		return rightA1 || rightA2;
	  });

	  let allSpan = document.querySelectorAll('span');
	  let specificSpan = Array.from(allSpan).filter(span => {
		const span1 = span.classList.contains('page-item_M4MDr') && span.classList.contains('pc');
		const span2 = span.classList.contains('tag-common_8ztfL') && span.classList.contains('c-gap-left-small');
		return span1 || span2;
	  });

	  let allB = document.querySelectorAll('b');
	  let specificB = Array.from(allB).filter(b => {
		const b1 = b.classList.contains('cur-tab') && b.classList.contains('c-color-t') && b.classList.contains('cur-tab_ReMW2') && b.classList.contains('s-tab-ps_RRh00');
		return b1;
	  });

	  let allI = document.querySelectorAll('i');
	  let specificI = Array.from(allI).filter(i => {
		const i1 = i.classList.contains('cr-title') && i.classList.contains('c-gap-bottom-xsmall');
		return i1;
	  });
	  ...
	  btn.addEventListener("change", e => {
	    if (e.detail === 'dark') {
		  ...
		  allP.forEach(element => {
			  element.style.backgroundColor = '#1d1d1f';
		      element.style.color = '#f5f5f7';
			  element.style.transition = "all 0.5s ease";
		  });
		  specificEm.forEach(element => {
			  element.style.color = '#f5f5f7';
			  element.style.transition = "all 0.5s ease";
	      });
		  pageButtonBg.forEach(element => {
			  element.style.backgroundColor = '#1d1d1f';
			  element.style.transition = "all 0.5s ease";
	      });
		  pageInputBg.forEach(element => {
			  element.style.backgroundColor = '#1d1d1f';
			  element.style.transition = "all 0.5s ease";
		  });
		  ...
		  specificDivs.forEach(element => {
			  element.style.backgroundColor = '#1d1d1f';
			  element.style.color = '#f5f5f7';
			  element.style.transition = "all 0.5s ease";
		  });
		  bottomDiv.forEach(element => {
			  element.style.backgroundColor = '#4f4f53';
			  element.style.transition = "all 0.5s ease";
		  });
		  specificA.forEach(element => {
			  element.style.backgroundColor = '#4f4f53';
			  element.style.color = '#c4c9fd';
			  element.style.transition = "all 0.5s ease";
		  });
		  rightA.forEach(element => {
			  element.style.color = '#c4c9fd';
			  element.style.transition = "all 0.5s ease";
		  });
		  specificSpan.forEach(element => {
			element.style.backgroundColor = '#4f4f53';
			element.style.color = '#c4c9fd';
			element.style.transition = "all 0.5s ease";
		  });
		  specificB.forEach(element => {
			element.style.color = '#fdfdfd';
			element.style.transition = "all 0.5s ease";
		  });
		  specificI.forEach(element => {
			element.style.color = '#fdfdfd';
			element.style.transition = "all 0.5s ease";
		  });
		  ...
		  pageButtonBg.forEach(element => {
			...
			element.style.color = '#2440b3';
			element.style.transition = "all 0.5s ease";
		  });
		  pageInputBg.forEach(element => {
			element.style.backgroundColor = '#ffffff';
			element.style.color = '#3951b3';
			element.style.transition = "all 0.5s ease";
		  });
		  inputColor.forEach(element => {
			element.style.color = '#1d1d1f';
		  ...
		  bottomDiv.forEach(element => {
			element.style.backgroundColor = '#f5f5f6';
			element.style.transition = "all 0.5s ease";
		  });
		  specificA.forEach(element => {
			element.style.backgroundColor = '#f5f5f5';
